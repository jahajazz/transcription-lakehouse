...
def check_timestamp_monotonicity(
    ...
    required_cols = ['episode_id', 'speaker', 'start_time', 'end_time']
...
def detect_integrity_violations(
    ...
    if 'speaker' in segments.columns:
        ...
        missing_speaker_indices = list(range(len(segments)))
        logger.warning(f"speaker column not found in {segment_type}s")
...
def detect_duplicates(
    ...
    text_groups = segments_with_normalized.groupby('_normalized_text')
    for normalized_text, group in text_groups:
